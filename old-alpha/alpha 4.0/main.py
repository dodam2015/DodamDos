...
while cmd != 'end':
    ...
    if cmd == 'dir':
        ...
        print('이 앱은 배타 버전이므로, 오류가 있을수 있습니다. 그래도 진행 하시겠습니까? (네, 아니요)')
        yesorno=input(':')
        if yesorno=='네':
            # GitHub 파일 URL
            enterappnamestore=input('앱 이름을 입력하세요:')
            url = 'https://raw.githubusercontent.com/dodam2015/dodamOSAPP/main/'+enterappnamestore+'.py'

            # 현재 스크립트가 있는 폴더 경로
            current_folder = os.path.dirname(os.path.abspath(__file__))

            # 저장할 파일 경로
            file_name = enterappnamestore+'.py'  # 원하는 파일 이름
